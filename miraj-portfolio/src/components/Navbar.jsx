...
const handleActive=(id)=>{
  ...
  return (
    ...
    >
      <div className="navbar px-5 max-w-7xl mx-auto">
        {/* Navbar Start */}
        <div className="navbar-start">
          <div className="dropdown md:hidden">
            ...
            <ul
              tabIndex={0}
              className="menu menu-sm dropdown-content bg-white/30 backdrop-blur-lg
              rounded-xl z-1 mt-3 w-36 p-2 shadow text-white"
            >
              {
                links.map((link,indx)=>(
               <li key={indx} ><a href={`#${link.id}`}>{link.name}</a></li>
          ...
          <div className="hidden md:flex items-center lg:gap-6 md:gap-3 text-white">
                  ...
                  {
                links.map((link,indx)=>(
            <a key={indx} onClick={()=>handleActive(link.id)} href={`#${link.id}`} className={`hover:text-cyan-300 ${isActive == link.id && "text-cyan-300"}`}>{link.name}</a>
